...
class CustomEmployee(NestedSet):
	...
	def autoname(self):
		...
		if self.user_id: self.employee = self.name
	...
	def validate(self):
		...
		self.share_todos() ###Custom Update
	...
	def share_todos(self):
		...
		if self.get_doc_before_save().todo_maintainer_:
		...
		if self.todo_maintainer_ and self.get_doc_before_save().todo_maintainer_ != self.todo_maintainer_:
