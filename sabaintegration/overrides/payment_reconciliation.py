...
class CustomPaymentReconciliation(PaymentReconciliation):
	...
	def get_unearned_entries(self):
		unearned_account = '2204 - Unearned Revenue - S'
		lft, rgt = frappe.db.get_value("Account", unearned_account , ["lft", "rgt"])
		children = frappe.get_all("Account", filters={"lft": [">=", lft], "rgt": ["<=", rgt]})
		names = [f"'{account['name']}'" for account in children]
		accounts_name_str = ", ".join(names)
		account_currency = frappe.db.get_value("Account" , self.receivable_payable_account , "account_currency")
		...
		journal_entries = frappe.db.sql(
			...
			select
				...
				t2.account_currency as currency , t2.account as account
			from
				`tabJournal Entry` t1, `tabJournal Entry Account` t2
			where
				t1.name = t2.parent and t1.docstatus = 1 and t2.docstatus = 1
				and t1.is_migrated = 0 
				and t2.account_currency = '{account_currency}'
				and t2.party_type = %(party_type)s and t2.party = %(party)s
				and t2.account IN ({accounts}) and {dr_or_cr} > 0 {condition}
	...
	@frappe.whitelist()
	def make_journal_entry(self ,account, amount, exchange_rate, is_advance, reference_journal_entry):
		journal_entry = frappe.new_doc("Journal Entry")
		journal_entry.voucher_type = "Journal Entry"
		journal_entry.company = self.company
		journal_entry.posting_date = nowdate()

		unearned_account = frappe._dict(
			{
				"account": account,
				"party_type": self.party_type,
				"party": self.party,
		...
		journal_entry.append("accounts", unearned_account)

		debtors_account = frappe._dict(
			{
		...
		journal_entry.append("accounts", debtors_account)
		journal_entry.save()
		journal_entry.submit()
		frappe.db.set_value("Journal Entry" , reference_journal_entry, "is_migrated", 1)
