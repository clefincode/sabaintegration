...
$.extend(sabaintegration, {
    set_cost_value(){
        for (let row of cur_frm.doc.costs){
            row.cost_value = cur_frm.doc.total * row.cost_percentage / 100;
    ...
    update_profit_loss(){
        cur_frm.doc.expected_profit_loss_value = cur_frm.doc.total - cur_frm.doc.total_costs_with_material_costs
        cur_frm.doc.base_expected_profit_loss_value = cur_frm.doc.expected_profit_loss_value * cur_frm.doc.conversion_rate;
        cur_frm.doc.expected_profit_loss = (cur_frm.doc.expected_profit_loss_value * 100) / cur_frm.doc.total;
    }
})