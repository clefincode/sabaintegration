...
erpnext.accounts.PaymentReconciliationController = class PaymentReconciliationController extends frappe.ui.form.Controller {
	...
	refresh() {
		...
		if (this.frm.doc.party && this.frm.doc.party_type == "Customer") {			
			this.frm.add_custom_button(__('Get Unearned  Entries'), () =>
				this.frm.trigger("get_unearned_entries") ,	
			);
		}
	...
	get_unearned_entries() {
		return this.frm.call({
			doc: this.frm.doc,
			method: 'get_unearned_entries',
			callback: (r) => {
				if(r.message.length == 0) frappe.throw({message: __("No Unearned Payments found for this party")});
				var child_table_data = r.message;
                var dialog = new frappe.ui.Dialog({
                    title: 'Unearned Entries',
                    fields: [
                        {
                            fieldname: 'field',
                            fieldtype: 'Table',
					...
					primary_action(values) {
						let rows = values.field
						let selected_rows = false;
						for(let f of rows){
							if(f.__checked){
								selected_rows = true
								cur_frm.call({
									...
									args:{
										...
										is_advance: f.is_advance,
										reference_journal_entry: f.reference_name,
									},
									callback: (r) => {
										dialog.hide();
									}
						...
						if(!selected_rows){
							dialog.hide();
							frappe.msgprint({message: __("No Rows Selected")});								
						}					
					}
                });
                dialog.show();			
