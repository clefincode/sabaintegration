...
sabaintegration.BDNSerialNoBatchSelector = class BDNSerialNoBatchSelector {
...
function get_rows_with_same_item_code(me) {
	const { frm: { doc: { stock_entries }}, item: { name, item_code }} = me;
	return stock_entries.filter(item => (item.name !== name) && (item.item_code === item_code))
}
